...
def main():
    ...
    parser = argparse.ArgumentParser(description="Estimate height map from normal map.")
    parser.add_argument("normal_map", type=str, help="Path to the normal map image file or URL.")
    args = parser.parse_args()

    normal_map: str = args.normal_map

    normal_map_img: np.ndarray = io.imread(normal_map)

    heights = estimate_height_map(normal_map_img, raw_values=True)

    figure, axes = plt.subplots(1, 2, figsize=(7, 3))
    _ = axes[0].imshow(normal_map_img)
